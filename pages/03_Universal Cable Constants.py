...
st.set_page_config(page_title="CABOTioN: Universal Cable Constants (UCC)", layout="centered", page_icon=icon)
...
st.title("Universal Cable Constants (UCC)")
#st.sidebar.markdown("# ... ")
...
with tab1:
    cable = st.selectbox("Select Cable Type",
                       options=["Two-wire", "Single Core", "Three Core", "Pipe Type"])
...
#with tab2:
    ...
    #st.markdown("""### Surface Operator""")
    #fig = px.line(np.absolute(df1), log_x=True, log_y=True)
